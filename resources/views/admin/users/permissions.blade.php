...
@push('styles')
    <style>
        ...
        html.dark .state-group {
            border-color: rgba(255, 255, 255, 0.1);
        }

        .state-group input[type="radio"] {
            display: none;
        }
        ...
        .state-group label:last-of-type {
            border-right: none;
        }
...
@section('content')
    ...
    <div class="flex items-center gap-4 mb-6">
        ...
        <div class="flex-1">
            ...
            <p class="text-[12.5px] text-[#9CA3AF] dark:text-[#555968] mt-0.5">
                ...
                @else
                    overrides take priority over the <span class="font-semibold capitalize">{{ $user->role }}</span> role
                    defaults
                @endif
            </p>
        </div>
        ...
        {{-- User badge --}}
        <div
            class="hidden md:flex items-center gap-2.5 px-4 py-2 rounded-[10px]
                bg-black/[0.03] dark:bg-white/[0.04]
                border border-black/[0.08] dark:border-white/[0.07]">
            <div
                class="w-7 h-7 rounded-[8px] flex items-center justify-center text-xs font-bold
    ...
    {{-- ── Legend ────────────────────────────────────────────────── --}}
    <div
        class="flex flex-wrap items-center gap-3 mb-5 px-4 py-3 rounded-[10px]
        ...
        <span class="inline-flex items-center gap-1.5 text-xs font-medium text-green-700 dark:text-green-400">
            <span
                class="w-3 h-3 rounded-sm bg-green-100 dark:bg-green-900/30 inline-block border border-green-300 dark:border-green-700"></span>
        ...
        <span class="inline-flex items-center gap-1.5 text-xs font-medium text-red-600 dark:text-red-400">
            <span
                class="w-3 h-3 rounded-sm bg-red-100 dark:bg-red-900/30 inline-block border border-red-300 dark:border-red-700"></span>
                        ...
                        <div class="perm-row">
                            ...
                            <div class="state-group" data-group="{{ $group }}">
                                <input type="radio" id="{{ $uid }}_inherit"
                                    name="overrides[{{ $perm->key }}]" value="inherit"
                                    {{ $currentState === 'inherit' ? 'checked' : '' }}>
                                <label for="{{ $uid }}_inherit">Inherit</label>

                                <input type="radio" id="{{ $uid }}_allow" name="overrides[{{ $perm->key }}]"
                                    value="allow" {{ $currentState === 'allow' ? 'checked' : '' }}>
                                <label for="{{ $uid }}_allow">Allow</label>

                                <input type="radio" id="{{ $uid }}_deny" name="overrides[{{ $perm->key }}]"
                                    value="deny" {{ $currentState === 'deny' ? 'checked' : '' }}>
