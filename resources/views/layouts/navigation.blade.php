...
<head>
    ...
    <style>
        ...
        /* Light mode → noise barely visible */
        html.dark body::before { opacity: 0.01; }
        ...
        html.dark .sidebar-glow::after { opacity: 0; }
        ...
        /* default = light mode (no .dark class) → show moon */
...
{{-- darkMode: 'class' in tailwind.config.js — 'dark' class = light mode (inverted), no class = dark mode --}}
<body class="bg-[#0D0D14] dark:bg-[#F0F2F8] text-[#E8E4DC] dark:text-[#1A1A2E] h-screen overflow-hidden">
    ...
    <div class="flex h-screen relative z-10">
        ...
        <aside id="sidebar"
            class="sidebar-glow
                   w-60 flex-shrink-0
                   bg-[#13131F] dark:bg-white
                   border-r border-white/[0.07] dark:border-black/[0.08]
                   flex flex-col
            ...
            <a href="{{ route('admin.dashboard.index') }}"
                class="flex items-center gap-[11px] px-5 py-6
                       border-b border-white/[0.07] dark:border-black/[0.07]
                ...
                <div class="overflow-hidden">
                    <div class="font-playfair text-base font-bold text-white dark:text-[#0D0D14] whitespace-nowrap">
                        {{ config('app.name', 'Mini Mart') }}
                    </div>
                    <span class="font-khmer text-[9px] text-[#6B7280] dark:text-[#9CA3AF] font-light block -mt-px">
                        ហាងលក់គ្រឿងទំនិញ
                    </span>
                </div>
            </a>
                ...
                <a href="{{ route('admin.dashboard.index') }}"
                    class="nav-active-bar flex items-center gap-2.5 px-3 py-2.5 rounded-[10px] no-underline
                           ...
                           {{ request()->routeIs('admin.dashboard.*')
                               ? 'bg-[rgba(0,48,135,0.25)] dark:bg-[rgba(0,48,135,0.1)] text-white dark:text-[#003087] border border-[rgba(0,48,135,0.35)] dark:border-[rgba(0,48,135,0.2)]'
                               : 'text-[#9CA3AF] dark:text-[#6B7280] hover:bg-white/[0.04] dark:hover:bg-[rgba(0,48,135,0.06)] hover:text-white dark:hover:text-[#003087] border border-transparent' }}">
            ...
            {{-- User Footer --}}
            <div class="px-3 py-3.5 border-t border-white/[0.07] dark:border-black/[0.08] flex-shrink-0">
                <div id="userTrigger" onclick="toggleUserMenu()"
                    class="flex items-center gap-2.5 px-3 py-2.5 rounded-[10px] cursor-pointer relative
                           bg-white/[0.04] dark:bg-black/[0.03]
                           border border-white/[0.07] dark:border-black/[0.08]
                           hover:bg-white/[0.07] dark:hover:bg-black/[0.06]
                    ...
                    <div class="flex-1 overflow-hidden min-w-0">
                        <div class="text-[13px] font-semibold text-white dark:text-[#0D0D14] truncate leading-tight">
                            {{ Auth::user()->name }}
                        </div>
                    ...
                    <div id="userDropdown"
                        class="hidden absolute bottom-[calc(100%+8px)] left-0 right-0 z-[100]
                               bg-[#1C1C2E] dark:bg-white
                               border border-white/[0.07] dark:border-black/[0.08]
                               rounded-xl p-1.5
                               shadow-[0_-8px_32px_rgba(0,0,0,0.4)] dark:shadow-[0_-8px_32px_rgba(0,0,0,0.12)]">

                        <a href="{{ route('profile.edit') }}"
                            class="flex items-center gap-2.5 px-3 py-2.5 rounded-lg text-[13px]
                                   text-[#9CA3AF] dark:text-[#6B7280] no-underline transition-all duration-150
                                   hover:bg-white/[0.04] dark:hover:bg-[rgba(0,48,135,0.06)]
                                   hover:text-white dark:hover:text-[#003087]">
                            👤 Profile Settings
                        </a>

                        <div class="h-px bg-white/[0.07] dark:bg-black/[0.07] my-1.5"></div>

                        <form method="POST" action="{{ route('logout') }}">
                            @csrf
                            <button type="submit"
                                class="flex items-center gap-2.5 w-full px-3 py-2.5 rounded-lg text-[13px]
                                       font-[inherit] text-[#9CA3AF] dark:text-[#6B7280]
        ...
        <div class="flex-1 flex flex-col overflow-hidden min-w-0">
            ...
            <header
                class="h-[60px] flex-shrink-0 flex items-center justify-between
                       px-7 max-md:px-4
                       border-b border-white/[0.07] dark:border-black/[0.08]
                       bg-[rgba(13,13,20,0.8)] dark:bg-[rgba(240,242,248,0.9)]
                ...
                <div class="flex items-center gap-3.5">
                    ...
                    <button onclick="openSidebar()"
                        class="hidden max-md:flex items-center justify-center w-9 h-9
                               ...
                               text-[#9CA3AF] dark:text-[#6B7280] text-xl cursor-pointer
                               transition-all duration-150
                               hover:bg-white/[0.04] dark:hover:bg-black/[0.05]
                               hover:text-white dark:hover:text-[#0D0D14]">
                        ☰
                    </button>
                    <div>
                        <div class="font-playfair text-xl font-bold text-white dark:text-[#0D0D14] leading-tight">
                            @yield('title', 'Dashboard')
                        </div>
                        <div class="text-xs text-[#6B7280] dark:text-[#9CA3AF] font-light mt-0.5">
                ...
                <div class="flex items-center gap-2.5">
                    ...
                    <div class="hidden md:flex items-center gap-2 h-9 px-3.5 rounded-[9px]
                                bg-white/[0.04] dark:bg-black/[0.04]
                                border border-white/[0.07] dark:border-black/[0.08]
                                font-mono-ibm text-xs text-[#9CA3AF] whitespace-nowrap">
                        <span id="clock-date"></span>
                        <span class="text-white/[0.07] dark:text-black/[0.15]">|</span>
                    ...
                    <button id="themeToggle" onclick="toggleTheme()" title="Toggle theme"
                        class="w-9 h-9 rounded-[9px] flex items-center justify-center text-base cursor-pointer
                               ...
                               hover:text-white dark:hover:text-[#0D0D14]
                               hover:border-white/[0.14] dark:hover:border-black/[0.14]
    ...
    <script>
        // ── Theme toggle ──────────────────────────────────────
        // Dark mode  = NO  'dark' class  (default, your original dark UI)
        // Light mode = HAS 'dark' class  (Tailwind dark: utilities activate)
        ...
        function applyTheme(theme) {
            ...
            if (theme === 'light') {
                html.classList.add('dark');    // Tailwind dark: = light UI
                if (icon) icon.textContent = '☀️';
            } else {
                html.classList.remove('dark'); // no dark class = dark UI (default)
